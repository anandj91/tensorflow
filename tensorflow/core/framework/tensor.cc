...
template <>
struct ProtoHelper<custom> {
  static void Fill(const custom* data, size_t n, TensorProto* proto) {
    proto->mutable_custom_val()->Reserve(n);
    for (size_t i = 0; i < n; ++i) {
      proto->mutable_custom_val()->AddAlreadyReserved(data[i].value);
    }
  }
};
...
template <>
TensorBuffer* FromProtoField<custom>(Allocator* a, const TensorProto& in,
                                       int64 n) {
  CHECK_GT(n, 0);
  Buffer<custom>* buf = new Buffer<custom>(a, n);
  uint32* data = buf->template base<uint32>();
  if (data == nullptr) {
    buf->Unref();
    return nullptr;
  }
  const int64 in_n = in.custom_val().size();
  auto begin = in.custom_val().begin();
  if (n <= in_n) {
    std::copy_n(begin, n, data);
  } else if (in_n > 0) {
    std::copy_n(begin, in_n, data);

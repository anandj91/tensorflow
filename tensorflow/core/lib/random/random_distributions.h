...
// Helper function to convert a 16-bit integer to a custom between [0..1).
PHILOX_DEVICE_INLINE custom Uint16ToCustom(uint16 x);
...
class UniformDistribution<Generator, custom> {
 public:
  ...
  ResultType operator()(Generator* gen) {
    ...
    ResultType result;
    for (int i = 0; i < kResultElementCount; ++i) {
      result[i] = Uint16ToCustom(sample[i]);
    }
    return result;
  }
};
